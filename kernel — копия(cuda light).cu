	...
	const Ray *r, double *t, int *id, Vec * hit, bool * isIntersect)
{
	 ...
	 __shared__ Vec hits[32];
	 __syncthreads ();
	 __shared__ int ids[32];
	 __shared__ bool isss[32];
	//Vec lockalHit;
	//bool intrs;
	//double  d;
	//float value;
...
*/
#else 
inline __device__  void Visible (const  triangle * Cobj,
...
int main (int argc, char *argv[])
{
	...
	camera cam (Ray (Vec (85, 55, 170), Vec (-0.45, -0.04, -1).normalization ()), Vec (w*.5135 / h));//front
