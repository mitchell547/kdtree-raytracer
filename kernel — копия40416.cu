...
struct triangle
{
	...
	inline __host__  triangle (Vec a1_, Vec a2_, Vec a3_, Vec c_, double reflect_) :
...
#ifdef host
	inline  __host__ bool		intersect (const Ray &r, Vec & hit) const
	{
	...
	inline  __host__ bool		SameSide (const Vec p1, const Vec p2, const Vec a, const Vec b)const
	{
	...
	inline  __host__ bool		PointInTriangle (const Vec p, const Vec a, const Vec b, const Vec c)const
	{
		if (SameSide (p, a, b, c) && 
			SameSide (p, b, a, c) && 
			SameSide (p, c, a, b))  
...
#else
inline  __device__ bool		intersect (const Ray &r, Vec & hit) const
	{
		double k = 0;								//plane coefficient
	...
	inline  __device__ bool		SameSide (const Vec p1, const Vec p2, const Vec a, const Vec b)const
	{
		Vec cp1 = (b - a).cross (p1 - a);
		Vec cp2 = (b - a).cross (p2 - a);
		double d = cp1.dot (cp2);
	...
	inline  __device__ bool		PointInTriangle (const Vec p, const Vec a, const Vec b, const Vec c)const
	{
		if (SameSide (p, a, b, c) && 
			SameSide (p, b, a, c) && 
			SameSide (p, c, a, b))  
...
struct world
{
	...
	world ();
	__host__ world (const unsigned int objCount_, const unsigned int lightsCount_,
...
inline __host__ __device__ char		toChar (double x) { return char (pow (clamp (x), 1 / 2.2) * 255); }
...
#ifdef host
inline __host__ void intersect (const triangle * objects, const unsigned int objCount,
...
inline __device__ void intersect (const triangle * objects, const  int * objCount,
	const Ray *r, double *t, int *id, Vec * hit, bool * isIntersect)
{
	 __shared__ double sdata[64];
	 __shared__ Vec hits[64];
	 __shared__ int ids[64];
	 __shared__ bool isss[64];
	Vec lockalHit;
	bool intrs;
	//double  d;
	//float value;
	*t = INF;
	...
	for (int i = 0; i < *objCount / blockDim.x; i++)
	{
		...
		isss[tid] = intrs;
		{
			sdata[tid] = lockalHit.distance (r->o);
			__syncthreads ();
...
#endif
////inline __host__  bool intersectHelper (const triangle * objects, const unsigned int objCount,
...
*/
#ifdef host
inline __host__  bool Visible (const  world & wrld, const Vec & hit, const Vec & light, const int & id)
{
...
*/
#endif
__device__  Vec RayTrace (const  triangle * Cobj,
...
#endif
	{
		...
		for (unsigned int sa = 0; sa < img->samps; ++sa)//quality of image
		{
			...
			Vec d = cam->cameraXangle*(((sx + .5 + dx) / 2 + x) / img->w - .5) +
				cam->cameraYangle*(((sy + .5 + dy) / 2 + y) / img->h - .5) +
				cam->cameraLocation.d;
			r = r + RayTrace (Cobj, cobj_COUNT, clights, clights_count, Ray (cam->cameraLocation.o + d * 140, d.normalization ()), 5)*(1. / img->samps);
		}
...
inline void RenderGPUHelper (const world & wrld,const camera & cam, Vec *c, const imgSettings & img)
{
	...
	cudaMemcpy (CUDA_objects, &obj, sizeO, cudaMemcpyHostToDevice);
	cudaMemcpy (CUDA_lights, &lights, sizeL, cudaMemcpyHostToDevice);
	...
	for (unsigned int y = 0; y < img.h; ++y) // Loop over image rows 
	{
		RenderGPU << <img.w, 32 >> >(CUDA_objects, CUDA_objects_count, CUDA_lights, CUDA_lights_count, cuda_cam, cuda_c /*+ y*img.w*/, cuda_img, y);
		fprintf (stderr, "\rRendering (%d spp) %5.2f%%", img.samps * 4, 100.*y / (img.h - 1));
	}
...
{
	triangle * objects;
	unsigned int objCount_ = model.TotalConnectedPoints;
	...
	for (unsigned int i = 0; i < objCount_; ++i)
	{
		...
		Vec a2 = Vec (model.Faces_Triangles[tr + 3], model.Faces_Triangles[tr + 4], model.Faces_Triangles[tr + 5]);
		Vec a3 = Vec (model.Faces_Triangles[tr + 6], model.Faces_Triangles[tr + 7], model.Faces_Triangles[tr + 8]);
		float inten = 0.8 - model.Faces_intensity[i];
